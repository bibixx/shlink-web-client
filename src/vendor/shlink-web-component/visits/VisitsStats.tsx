...
export const VisitsStats: FC<VisitsStatsProps> = (props) => {
  ...
  const { os, browsers, referrers, countries, cities, citiesForMap, visitedUrls, queryParams } = useMemo(
    () => processStatsFromVisits(normalizedVisits),
    [normalizedVisits],
  );
  const hasQueryParams = useMemo(() => {
    const hasNoSrc = queryParams['no_src'] != null;
    if (!hasNoSrc) {
            ...
            <Row>
              <Routes>
                ...
                <Route
                  ...
                  element={(
                    <VisitsSectionWithFallback showFallback={visits.length === 0}>
                      {hasQueryParams && <div className={clsx('mt-3 col-lg-6', { 'col-xl-4': !isOrphanVisits })}>
                        <DoughnutChartCard title="Tracking query param" stats={queryParams} prevStats={processedPrevStats.queryParams} />
                      </div>}
...
function filterFalsey<T>(value: T | false | null | undefined): value is T {
  return !!value;
}
