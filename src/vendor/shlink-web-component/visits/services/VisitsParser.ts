...
const updateQueryParamsForVisit = (queryParamsStats: Stats, visit: NormalizedVisit) => {
  if (visit.visitedUrl == null) {
    return;
  }

  const { visitedUrl } = visit;
  const queryParams = new URL(visitedUrl).searchParams;
  const src = queryParams.get(TRACKING_PARAM);

  if (src) {
    queryParamsStats[src] ??= 0;
    queryParamsStats[src] += 1;
  } else {
    queryParamsStats['no_src'] ??= 0;
    queryParamsStats['no_src'] += 1;
  }
};
...
export const processStatsFromVisits = (visits: NormalizedVisit[]) => visits.reduce(
  (stats: VisitsStats, visit: NormalizedVisit) => {
    ...
    updateQueryParamsForVisit(stats.queryParams, visit);
